...
Native modules note: `@notifee/react-native` (drives the persistent Android
notification panel in `lib/bulkStatusPanel.android.ts`) needs a development
build (`npx expo run:android` or an EAS build) — it will not load in Expo Go.
The panel's fold-wheel artwork is generated by
`node tools/generate_notification_icons.mjs` → `assets/images/fold-wheel/`
(needs node-canvas: `npm i canvas`, run on the owner's machine).
