...
Animation note: the timer screen currently uses the pure-JS `FermentationScene`.
The Skia version (`@shopify/react-native-skia`) crashes on launch under the New
Architecture and is being fixed on `claude/skia-fix` — see `docs/SKIA-HANDOFF.md`.
