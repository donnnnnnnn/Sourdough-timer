...
import { diagnose, type ClassifierInput } from '@/model/classifier';
...
// Maps the three quick-log options to classifier-compatible diagnosis
...
// Tiebreaker questions — shown only on low-confidence photo results
const Q_SHAPE = {
  key: 'shape' as const,
...
type TiebreakerAnswers = {
  shape?: string;
  crust?: string;
  texture?: string;
};

function pickTiebreakerQuestions(diag: Diagnosis, confidence: number) {
...
// Placeholder image probabilities — real ML model slots in here later
function mockCrumbProbs(): Record<FermentationState, number> {
  return {
    under_fermented: 0.15,
    slightly_under: 0.12,
    properly_fermented: 0.60,
    slightly_over: 0.08,
    over_fermented: 0.05,
  };
}
...
export default function LogScreen() {
  ...
  // ── Quick-log: one tap saves and goes home ───────────────────────
  function handleQuickLog(diagnosis: Diagnosis) {
    saveLog('Classical', 'Full Body', diagnosis);
    router.push('/');
  }
  ...
  function runClassifier(extra?: TiebreakerAnswers) {
    setStep('analysing');
    // Simulate async model inference (replace with real TFLite call later)
    setTimeout(() => {
      const answers = extra ?? {};
      const input: ClassifierInput = {
        crumbProbs: mockCrumbProbs(),
        shapeFlat: answers.shape === 'flat',
        crustPale: answers.crust === 'pale',
        gummyDetected: answers.texture === 'gummy',
        evenHoles: answers.texture === 'clean',
        tunnelingDetected: false,
        topHeavyHoles: answers.shape === 'pyramidal',
        megaPocketsNearCrust: false,
        shoulderProfile: (answers.shape as ClassifierInput['shoulderProfile']) ?? 'unknown',
        glutenStrandsInBloom: false,
        bubblesInBloom: answers.shape === 'falling',
  ...
  function handleTiebreakerAnswer(key: keyof TiebreakerAnswers, value: string) {
    ...
    const questions = diagResult ? pickTiebreakerQuestions(diagResult.diagnosis, diagResult.confidence) : [];
    const allAnswered = questions.every(q => updated[q.key as keyof TiebreakerAnswers]);
    if (allAnswered) {
      runClassifier(updated);
    }
  }

  function handleSaveResult() {
    if (!diagResult) return;
    saveLog('Classical', 'Full Body', diagResult.diagnosis as Diagnosis);
    router.push('/');
  }
  ...
  if (step === 'analysing') {
    return (
      <View style={{ flex: 1, backgroundColor: C.bg, alignItems: 'center', justifyContent: 'center', gap: 20 }}>
        {photoUri && (
          <Image source={{ uri: photoUri }} style={{ width: 200, height: 200, borderRadius: 20, opacity: 0.6 }} />
        )}
  ...
  if (step === 'result' && diagResult) {
    ...
    const isLowConf = diagResult.confidence < 0.75;
    const tbQuestions = isLowConf ? pickTiebreakerQuestions(diagResult.diagnosis, diagResult.confidence) : [];

    if (isLowConf && step === 'result') {
      // Switch to tiebreaker
      return (
        <ScrollView style={{ flex: 1, backgroundColor: C.bg }} contentContainerStyle={{ padding: 24, paddingBottom: 48 }}>
          ...
          {tbQuestions.map((q) => (
            <View key={q.key} style={{ marginBottom: 24 }}>
              ...
              <View style={{ gap: 8 }}>
                {q.options.map((opt) => {
                  const active = tbAnswers[q.key as keyof TiebreakerAnswers] === opt.value;
                  return (
                    <TouchableOpacity
                      key={opt.value}
                      onPress={() => handleTiebreakerAnswer(q.key as keyof TiebreakerAnswers, opt.value)}
                      activeOpacity={0.7}
                      style={{
                      ...
                      <Text style={{ color: active ? C.accent : C.text, fontSize: 15, fontWeight: '600' }}>
                        {opt.label}
                      </Text>
                    </TouchableOpacity>
                  );
    ...
    return (
      <ScrollView style={{ flex: 1, backgroundColor: C.bg }} contentContainerStyle={{ padding: 24, paddingBottom: 48 }}>
        ...
        {photoUri && (
          <Image source={{ uri: photoUri }} style={{ width: '100%', height: 200, borderRadius: 20, marginBottom: 20 }} />
        )}
        ...
        <View style={{
          ...
          <Text style={{ color: C.text, fontSize: 26, fontWeight: '800', letterSpacing: -0.3, marginBottom: 8 }}>
            {copy.title}
          </Text>
          <Text style={{ color: C.text, fontSize: 16, lineHeight: 24, marginBottom: 16 }}>
            {copy.oneLiner}
          </Text>
          ...
          {whyOpen && (
            <Text style={{ color: C.textMuted, fontSize: 14, lineHeight: 22, marginTop: 14 }}>
              {copy.expandedWhy}
            </Text>
          )}
        </View>
        ...
        <TouchableOpacity
          onPress={() => { setStep('photo'); setDiagResult(null); setPhotoUri(null); setWhyOpen(false); setTbAnswers({}); }}
          activeOpacity={0.7}
          style={{
    ...
    return (
      <ScrollView style={{ flex: 1, backgroundColor: C.bg }} contentContainerStyle={{ padding: 24, paddingBottom: 48 }}>
        ...
        <Text style={{ color: C.textMuted, fontSize: 15, marginBottom: 32, lineHeight: 22 }}>
          Take a photo of the crumb cross-section after slicing.
        </Text>
